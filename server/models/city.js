...
var City, addAPIKey, addCityKeys, americano, async, callWithRetry, cityUrl, delayRetry, http, httpGet, maxRetry;
...
callWithRetry = function(url, callback, count) {
  if (!count) {
    count = 1;
  }
  return httpGet(url, null, function(infos, err) {
    if ((!err) && (!infos) && count < maxRetry) {
      return setTimeout((function() {
        return callWithRetry(url, callback, count + 1);
      }), delayRetry);
    } else {
      return callback(infos, err);
    }
  });
};
...
City.fullCity = function(city, mainCallback) {
  ...
  return async.series([
    (function(callback) {
      return callWithRetry(addAPIKey(weatherUrl), function(weather, err) {
        if (!err) {
      ...
      return callWithRetry(addAPIKey(forecastUrl), function(forecast, err) {
        if (!err) {
      ...
      return callWithRetry(addAPIKey(dayForecastUrl), function(forecast, err) {
        if (!err) {
